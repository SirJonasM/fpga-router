...
pub fn report_violations(
    ...
    let mut min_delay_path: (f64, f64, Node, Node, Vec<TimingNode>) = (
        f64::INFINITY,
        0.0,
        Node {
            tile: (0, 0),
    ...
    let mut max_delay_path: (f64, f64, Node, Node, Vec<TimingNode>) = (
        f64::INFINITY,
        0.0,
        Node {
            tile: (0, 0),
    ...
    log::info!(
        "Amount of paths analyzed: {} min paths, {} max paths",
        min_path.len(),
        max_path.len()
    );
    log::trace!("--- Min Paths ---");
    ...
    for path in min_path.iter() {
        ...
        log::trace!(
            "   Path: {}",
            path_trace.iter().map(|p| p.to_string()).collect::<Vec<_>>().join(" -> ")
        );
        log::trace!("");
    }
    ...
    if hold_slack < 0.0 {
        ...
        log::info!("Hold Slack: {:.2} ps", hold_slack);
    }
    ...
    if setup_slack < 0.0 {
        ...
        log::info!("Setup Slack: {:.2} ps", setup_slack);
    }
